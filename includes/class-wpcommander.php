...
final class WPCommander {
	...
	private function __construct() {
		...
		$this->media_transfer = new WPCommander_Media_Transfer( $this->mutations );
	...
	public function register_rest_routes(): void {
		...
		register_rest_route(
			...
			array(
				'methods'             => WP_REST_Server::CREATABLE,
				'callback'            => array( $this, 'rest_import_chat_images' ),
				'permission_callback' => array( $this->media_transfer, 'can_import' ),
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => WP_REST_Server::CREATABLE,
				'callback'            => array( $this, 'rest_get_media_links' ),
				'permission_callback' => array( $this->media_transfer, 'can_export' ),
			)
		);
	...
	private function get_media_import_schema(): array {
		return array(
			'type'       => 'object',
			'required'   => array( 'openaiFileIdRefs' ),
			'properties' => array(
				'openaiFileIdRefs' => array(
					'type'        => 'array',
					'minItems'    => 1,
					'maxItems'    => 10,
					'items'       => array( 'type' => 'string' ),
					'description' => 'One to ten current-conversation images uploaded by the user or generated by ChatGPT. The Actions runtime replaces these strings with temporary OpenAI file-reference objects.',
				),
				'parentId' => array( 'type' => 'integer', 'minimum' => 0, 'description' => 'Optional WordPress post ID to attach the imported media to.' ),
			),
		);
	}

	private function get_media_links_schema(): array {
		return array(
			'type'       => 'object',
			'required'   => array( 'ids' ),
			'properties' => array(
				'ids' => array(
					...
					'description' => 'One to ten WordPress media attachment IDs to return as usable image and download URLs.',
				),
			),
		);
	}
	...
	public function rest_import_chat_images( WP_REST_Request $request ) {
		$result = $this->media_transfer->import_chat_images( (array) $request->get_json_params() );
		return is_wp_error( $result ) ? $result : rest_ensure_response( $result );
	}

	public function rest_get_media_links( WP_REST_Request $request ) {
		$result = $this->media_transfer->get_media_links( (array) $request->get_json_params() );
		return is_wp_error( $result ) ? $result : rest_ensure_response( $result );
	}
	...
	public function get_manifest_data(): array {
		...
		return array(
			...
			'capabilities'      => array(
				...
				array(
					'id'          => 'media-transfer',
					'label'       => __( 'Transfer chat images', 'wpcommander' ),
					'description' => __( 'Import current conversation images into the WordPress Media Library and return usable WordPress image URLs.', 'wpcommander' ),
					'access'      => $this->media_transfer->can_import() ? 'write' : 'read',
					'source'      => 'WPCommander',
				),
				array(
...
8. When the user wants an image from the current conversation used in WordPress, including an image they uploaded or ChatGPT generated, call importChatImagesToWordPress first. Never invent openaiFileIdRefs; pass the actual current-conversation file(s).
9. Use getWordPressImageLinks when you need usable URLs, dimensions, or WordPress image variants for existing media attachments.
10. Use getWPCommanderDiagnostics only for connection/access troubleshooting.
	...
	private function get_openapi_schema(): array {
		...
		return array(
			...
			'paths'   => array(
				...
				'/wp-json/wpcommander/v1/media/import-chat-images' => array(
					'post' => array(
						'operationId' => 'importChatImagesToWordPress',
						'summary' => 'Import conversation images into WordPress media',
						'description' => 'Import one to ten images from the current chat, including user uploads and ChatGPT-generated images. Requires Edit site or Full control and WordPress media upload permission.',
						'security' => $security,
						'requestBody' => array( 'required' => true, 'content' => array( 'application/json' => array( 'schema' => $this->get_media_import_schema() ) ) ),
						'responses' => $object_response,
					),
				),
				'/wp-json/wpcommander/v1/media/links' => array(
					'post' => array(
						'operationId' => 'getWordPressImageLinks',
						'summary' => 'Get usable URLs for WordPress images',
						'description' => 'Return image, download, dimensions, metadata, and common WordPress size URLs for up to ten media attachment IDs.',
						'security' => $security,
						'requestBody' => array( 'required' => true, 'content' => array( 'application/json' => array( 'schema' => $this->get_media_links_schema() ) ) ),
						'responses' => $object_response,
					),
				),
